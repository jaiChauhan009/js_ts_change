...
export class EventProcessorService {
  ...
  private handleEvent(event: NotifierEvent): boolean {
    if (event.address === this.contractGasService) {
      ...
      if (validEvent) {
        this.logger.debug('Received Gas Service event from Dharitri:');
    ...
    if (event.address === this.contractGateway) {
      ...
      if (validEvent) {
        this.logger.debug('Received Gateway event from Dharitri:');
        this.logger.debug(JSON.stringify(event));
      }
    ...
    if (event.address === this.contractIts) {
      ...
      if (validEvent) {
        this.logger.debug('Received ITS event from Dharitri:');
        this.logger.debug(JSON.stringify(event));
      }
